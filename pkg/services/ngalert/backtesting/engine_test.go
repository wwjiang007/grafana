	...
	t.Run("creates data evaluator", func(t *testing.T) {
		...
		for _, testCase := range testCases {
			t.Run(testCase.name, func(t *testing.T) {
				e, err := newBacktestingEvaluator(context.Background(), evalFactory, nil, testCase.condition, eval.LoadedAlertsReader{})
				if testCase.error {
					require.Error(t, err)
					return
				}
...
func TestEvaluatorTest(t *testing.T) {
	...
	backtestingEvaluatorFactory = func(ctx context.Context, evalFactory eval.EvaluatorFactory, user *user.SignedInUser, condition models.Condition, r expr.LoadedMetricsReader) (backtestingEvaluator, error) {
