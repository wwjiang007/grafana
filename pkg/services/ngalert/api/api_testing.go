...
func (srv TestingApiSrv) RouteTestGrafanaRuleConfig(c *contextmodel.ReqContext, body apimodels.PostableExtendedRuleNodeExtended) response.Response {
	...
	evaluator, err := srv.evaluator.CreateWithLoadedMetrics(eval.NewContext(c.Req.Context(), c.SignedInUser), rule.GetEvalCondition(), &eval.LoadedAlertsReader{})
	if err != nil {
...
func (srv TestingApiSrv) RouteEvalQueries(c *contextmodel.ReqContext, cmd apimodels.EvalQueriesPayload) response.Response {
	...
	evaluator, err := srv.evaluator.CreateWithLoadedMetrics(eval.NewContext(c.Req.Context(), c.SignedInUser), cond, &eval.LoadedAlertsReader{})
